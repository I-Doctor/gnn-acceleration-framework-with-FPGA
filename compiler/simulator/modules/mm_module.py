...
class MmModule:
    ...
    def run_mm(self, inst_param, Mempool):
        ...
        if self.b: # 加bias
            bias_data = Mempool.read_mempool("bias", 0, self.bias_start_address, N * self.output_address_per_feature)
            bias_data = bias_data.reshape((N, self.output_address_per_feature, OCP))
            feature_out = feature_out + bias_data
        ...
        if self.a: # 加另一个矩阵
            ...
            feature_out = feature_out + add_matrix
